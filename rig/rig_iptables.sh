...
#ssh
iptables -A INPUT -i enp7s0 -p tcp -m tcp --dport 22 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
iptables -A INPUT -i enp7s0 -p tcp -m tcp --dport 22 -j ACCEPT
...
#dhcp
iptables -A INPUT -i enp7s0 -p udp -m udp --dport 67 -j ACCEPT
iptables -A INPUT -i enp7s0 -p tcp -m tcp --dport 67 -j ACCEPT
